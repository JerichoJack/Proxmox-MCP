...
class ProxmoxMCPServer:
    ...
    def __init__(self):
        ...
        # Initialize Proxmox API manager
        ...
        logger.info(f"PVE nodes: {len(self.config.pve_nodes)} ({len(self.api_manager.pve_clients)} connected)")
        logger.info(f"PBS nodes: {len(self.config.pbs_nodes)} ({len(self.api_manager.pbs_clients)} connected)")
            ...
            if uri == "proxmox://config":
                return json.dumps({
                    ...
                    "connection_summary": {
                        ...
                        "total_connected": len(self.api_manager.pve_clients) + len(self.api_manager.pbs_clients),
                        "connection_health": "healthy" if len(self.api_manager.get_all_clients()) > 0 else "no_connections"
                    }
                }, indent=2)
                ...
                for node_name in self.config.pve_nodes:
                    ...
                    node_details[node_name] = {
                        ...
                        "api_available": node_name in self.api_manager.pve_clients
                    }
                ...
                for node_name in self.config.pbs_nodes:
                    ...
                    node_details[node_name] = {
                        ...
                        "api_available": node_name in self.api_manager.pbs_clients
                    }
                ...
                return json.dumps({
                    ...
                    "health_summary": {
                        "overall_status": "healthy" if len(all_clients) > 0 else "no_connections",
...
Guide users through systematic incident response with clear priorities and actionable steps."""
    ...
    async def _get_cluster_status(self, args: Dict[str, Any]) -> Sequence[TextContent]:
        ...
        for node_name, client in self.api_manager.pve_clients.items():
            try:
        ...
        status = {
            ...
            "summary": {
                ...
                "pve_nodes_connected": len(self.api_manager.pve_clients),
                "pbs_nodes_connected": len(self.api_manager.pbs_clients),
    ...
    async def _get_nodes(self, args: Dict[str, Any]) -> Sequence[TextContent]:
        ...
        results = await self.api_manager.aggregate_results(get_nodes_from_client)
    ...
    async def _get_node_status(self, args: Dict[str, Any]) -> Sequence[TextContent]:
        ...
        client = self.api_manager.get_client(node_name)
        if not client:
            return [TextContent(
                type="text",
                text=f"Node '{node_name}' not found in connected clients. Available nodes: {list(self.api_manager.get_all_clients().keys())}"
            )]
    ...
    async def _get_vms(self, args: Dict[str, Any]) -> Sequence[TextContent]:
        ...
        if node:
            ...
            client = self.api_manager.get_client(node)
            if not client:
                return [TextContent(
                    type="text",
            ...
            results = await self.api_manager.aggregate_results(get_vms_from_client)
    ...
    async def _get_lxcs(self, args: Dict[str, Any]) -> Sequence[TextContent]:
        ...
        if node:
            ...
            results = await self.api_manager.aggregate_results(get_lxcs_from_client)
    ...
    async def _get_storage(self, args: Dict[str, Any]) -> Sequence[TextContent]:
        ...
        if node:
            ...
            results = await self.api_manager.aggregate_results(get_storage_from_client)
    ...
    async def _get_backup_info(self, args: Dict[str, Any]) -> Sequence[TextContent]:
        ...
        if node:
            ...
            results = await self.api_manager.aggregate_results(get_backup_from_client)
    ...
    async def _get_network_info(self, args: Dict[str, Any]) -> Sequence[TextContent]:
        ...
        if node:
            ...
            results = await self.api_manager.aggregate_results(get_network_from_client)
    ...
    async def _check_node_health(self, args: Dict[str, Any]) -> Sequence[TextContent]:
        ...
        detailed_results = []
        if target_node:
            client = self.api_manager.get_client(target_node)
            if client:
                try:
            ...
            # Check all nodes
            for node_name, client in self.api_manager.get_all_clients().items():
                try:
                    node_status = await client.get_node_status()
                    detailed_results.append({
                        "node": node_name,
